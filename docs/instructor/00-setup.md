...
- The Node server and helper scripts load the repository `.env` with override enabled, so stale exported `LANGFUSE_*`, `ANTHROPIC_*`, or `DATASET_NAME` values from another local project should not win. Have learners edit `.env` when they need to switch keys or projects.
